...
listdir(Pane *pane)
{
	...
	set_direntr(
		pane, entry, dir, pane->filter); /* create array of entries ----- YOU NEED TO CREATE A FUNCTION LIKE THIS
																				IN ORDER TO SET PANES ENTRY AND THEN refresh_pane(pane) 
																				CHECK LINE 1767*/
...
set_panes(void)
{
	...
	// uncomment to set the first pane as paneone defined in config.h
	//strncpy(panes[Left].dirn, paneone, MAX_P);
	...
	strncpy(panes[Right].dirn, panetwo, MAX_P); // replace string with panetwo defined in config.h
	// uncomment the below 2 lines in order to take the second pane as the second argument to sfm
